...
\begin{equation}
p(x) \triangleq \theta^{\mathbb{I}(x=1)}(1-\theta)^{\mathbb{I}(x=0)}
\end{equation}
\end{definition}
...
\begin{equation}
p(k) \triangleq \dbinom{n}{k}\theta^k(1-\theta)^{n-k}
\end{equation}
\end{definition}
...
\begin{definition}
The Bernoulli distribution can be used to model the outcome of one coin tosse. To model the outcome of tossing a K-sided die, let $\vec{x} =(\mathbb{I}(x=1),\cdots,\mathbb{I}(x=K)) \in \{0,1\}^K$ be a random vector(this is called \textbf{dummy encoding} or \textbf{one-hot encoding}), then we say $X$ has a \textbf{multinoulli distribution}(or \textbf{categorical distribution}), written as $X \sim \text{Cat}(\theta)$. The pmf is given by: 
\begin{equation}
p(\vec{x}) \triangleq \prod\limits_{k=1}^K\theta_j^{\mathbb{I}(x_j=1)}
\end{equation}
\end{definition}

\begin{definition}
Suppose we toss a K-sided die $n$ times. Let $\vec{x} =(x_1,x_2,\cdots,x_K) \in \{0,1,\cdots,n\}^K$ be a random vector, where $x_j$ is the number of times side $j$ of the die occurs, then we say $X$ has a \textbf{multinomial distribution}, written as $X \sim \text{Mu}(n, \vec{\theta})$. The pmf is given by 
\begin{equation}
p(\vec{x}) \triangleq \dbinom{n}{x_1 \cdots x_k} \prod\limits_{k=1}^K\theta_j^{x_j} \quad \text{where} \dbinom{n}{x_1 \cdots x_k} \triangleq \dfrac{n!}{x_1!x_2! \cdots x_K!}
\end{equation}
\end{definition}

Bernoulli distribution is just a special case of a Binomial distribution with $n=1$, and so is multinoulli distribution as to multinomial distribution. See Table ~\ref{tab:multinomial} for a summary.

\begin{longtable}{p{2cm}p{1cm}p{1cm}p{4.6cm}}
\caption{Summary of the multinomial and related distributions.} \label{tab:Summary-distribution}\\  
Name & K & n & X \\
...
\begin{definition}
We say that $X \in \{0,1,2,\cdots\}$ has a \textbf{Poisson distribution} with parameter $\lambda>0$, written $X \sim \text{Poi}(\lambda)$, if its pmf is
\begin{equation}
...
\caption{Summary of Bernoulli, binomial multinoulli and multinomial distributions.} \label{tab:multinomial}\\
\hline\noalign{\smallskip}
...
\begin{equation}
\Gamma(x) \triangleq \int_0^\infty u^{x-1}e^{-u}dx
\end{equation}
...
\begin{figure}[hbtp]
\centering
    \includegraphics[scale=.70]{pdfs-for-NTL.png}
...
\begin{figure}[hbtp]
\centering
    \includegraphics[scale=.60]{robustness.png}
...
\begin{figure}[hbtp]
\centering
    \includegraphics[scale=.60]{gamma-distribution.png}
...
\begin{table}
\caption{Summary of Beta distribution}
\centering
\begin{tabular}{ccccccc}
\hline\noalign{\smallskip}
Name & Written as & $X$ & $f(x)$ & $\mathbb{E}[X]$ & mode & $\text{var}[X]$ \\
...
\begin{figure}[hbtp]
\centering
    \includegraphics[scale=.50]{Pareto-distribution.png}
...
\begin{equation}
Cov[X,Y] \triangleq \mathbb{E}\left[(X-\mathbb{E}[X])(Y-\mathbb{E}[Y])\right]=\mathbb{E}[XY]-\mathbb{E}[X]\mathbb{E}[Y]
\end{equation}
\end{definition}
...
\begin{figure}[hbtp]
\centering
    \includegraphics[scale=.60]{2d-Gaussions.png}
...
\begin{equation}
B(\alpha) \triangleq \dfrac{\prod\_{k=1}^K \Gamma(\alpha_k)}{\Gamma(\alpha_0)} \text{ where } \alpha_0 \triangleq \sum_{k=1}^K \alpha_k
\end{equation}
...
\begin{figure}[hbtp]
\centering
    \includegraphics[scale=.60]{3d-Dirichlet.png}
...
\begin{figure}[hbtp]
\centering
    \includegraphics[scale=.60]{5d-Dirichlet.png}
\caption{Samples from a 5-dimensional symmetric Dirichlet distribution for different parameter values. (a) $\vec{\alpha}=(0.1,\cdots,0.1)$. This results in very sparse distributions, with many 0s. (b) $\vec{\alpha}=(1,\cdots,1)$. This results in more uniform (and dense) distributions.} 
...
If $X$ is continuous, we cannot use Equation \eqref{eqn:transformation-discrete} since $p_X(x)$ is a density, not a pmf, and we cannot sum up densities. Instead, we work with cdf’s, and write
\begin{equation}
F_Y(y)=P(Y \leq y)=P(g(X) \leq y)=\int\limits_{g(X) \leq y} f_X(x)dx
\end{equation}
...
\begin{theorem}
Given $N$ random variables $X_1,X_2,\cdots,X_N$, each variable is \textbf{independent and identically distributed}(\textbf{iid} for short), and each has the same mean $\mu$ and variance $\sigma^2$, then
\begin{equation}
\dfrac{\sum\limits_{i=1}^n X_i-N\mu}{\sqrt{N}\sigma} \sim \mathcal{N}(0,1)
\end{equation}
this can also be written as
\begin{equation}
\dfrac{\bar{X}-\mu}{\sigma/\sqrt{N}} \sim \mathcal{N}(0,1) \text{ where } \bar{X} \triangleq \dfrac{1}{N}\sum\limits_{i=1}^n X_i
\end{equation}
\end{theorem}
...
\begin{equation}
\mathbb{E}[g(X)]=\int g(x)p(x)dx \approx \dfrac{1}{S}\sum\limits_{s=1}^S f(x_s) \text{ , where } x_s \sim p(X)
\end{equation}
